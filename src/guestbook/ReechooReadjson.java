...
public class ReechooReadjson {
	
	SimpleDateFormat aSF = new SimpleDateFormat("yyyy-MM-dd'T'hh:mm:ss");
	
	private static final Logger log =  Logger.getLogger(ReechooReadjson.class.getName());
...
if (count1>1)
 jarray = json.getJSONObject("query").getJSONObject("results").getJSONArray("item");
else if (count1==1)
{
	jarray.put(json.getJSONObject("query").getJSONObject("results").getJSONObject("item"));
